...
CREATE VIEW [NetO].[VwWG_KELLEYBLUEBOOK_RESPONSE]
AS
   SELECT
      x.[LNUM],
      x.[DBID],
      ...
      x.[STATUS_CODE],
      x.[STATUS_MESSAGE],
      HASHBYTES('SHA2_256', x.[VEHICLE_ID]) AS [VEHICLE_ID],
      x.[TRADE_IN_VAL],
      x.[RETAIL_VAL],
      x.[WHOLESALE_VAL],
      x.[INVOICE_VAL],
      x.[CYLINDERS],
      HASHBYTES('SHA2_256', CAST(x.[CC] AS NVARCHAR(50))) AS [CC],
      x.[STROKE],
      x.[CATEGORY],
      x.[S_GENERIC_BODY_STYLE],
      A0.[Descript] AS [S_GENERIC_BODY_STYLE_X]
