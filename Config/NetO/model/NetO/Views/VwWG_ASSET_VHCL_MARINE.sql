...
CREATE VIEW [NetO].[VwWG_ASSET_VHCL_MARINE]
AS
   SELECT
      x.[LNUM],
      x.[ASSETID],
      x.[S_MARINE_TYPE],
      A0.[Descript] AS [S_MARINE_TYPE_X],
      HASHBYTES('SHA2_256', x.[BOAT_NAME]) AS [BOAT_NAME],
      x.[NET_WEIGHT],
      x.[S_PROPULSION_TYPE],
      A1.[Descript] AS [S_PROPULSION_TYPE_X],
      x.[HAILING_PORT],
      HASHBYTES('SHA2_256', x.[MOORING_ADDR1]) AS [MOORING_ADDR1],
      HASHBYTES('SHA2_256', x.[MOORING_ADDR2]) AS [MOORING_ADDR2],
      HASHBYTES('SHA2_256', x.[MOORING_CITY]) AS [MOORING_CITY],
      HASHBYTES('SHA2_256', x.[MOORING_STATE]) AS [MOORING_STATE],
      HASHBYTES('SHA2_256', x.[MOORING_ZIP]) AS [MOORING_ZIP],
      x.[TRAILER_ASSETID],
      x.[S_FUEL_TYPE],
      A2.[Descript] AS [S_FUEL_TYPE_X],
      x.[BEAM],
      x.[MARINE_LENGTH],
      x.[S_HULL_MATERIAL],
      A3.[Descript] AS [S_HULL_MATERIAL_X],
      x.[S_PRIMARY_USE],
      A4.[Descript] AS [S_PRIMARY_USE_X],
      x.[S_ENGINE_DRIVE],
      A5.[Descript] AS [S_ENGINE_DRIVE_X],
      x.[IS_DOCUMENTED_VESSEL],
      HASHBYTES('SHA2_256', x.[USCG_OFFICIAL_NUMBER]) AS [USCG_OFFICIAL_NUMBER],
      x.[S_MANUF_TYPE],
      A6.[Descript] AS [S_MANUF_TYPE_X],
      x.[TOILETATTACHED],
      x.[MATLOTHDESC],
      x.[FUELOTHDESC],
      x.[PROPOTHDESC],
      HASHBYTES('SHA2_256', x.[MOORING_SLIPNO]) AS [MOORING_SLIPNO],
      HASHBYTES('SHA2_256', x.[MOORING_FACILITY]) AS [MOORING_FACILITY],
      HASHBYTES('SHA2_256', CAST(x.[MOORING_MRENT] AS NVARCHAR(50))) AS [MOORING_MRENT]
