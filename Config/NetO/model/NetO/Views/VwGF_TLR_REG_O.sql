...
CREATE VIEW [NetO].[VwGF_TLR_REG_O]
AS
   SELECT
      x.[LNUM],
      x.[BNUM],
      ...
      x.[O_DATE],
      x.[OFFICER_TITLE],
      HASHBYTES('SHA2_256', CAST(x.[NUMBER_OF_SHARES] AS NVARCHAR(50))) AS [NUMBER_OF_SHARES],
      HASHBYTES('SHA2_256', x.[REG_O_BORROWER]) AS [REG_O_BORROWER],
      HASHBYTES('SHA2_256', x.[EMPLOYEE_BORROWER]) AS [EMPLOYEE_BORROWER],
      x.[S_EMP_REGO_TYPE],
      A0.[Descript] AS [S_EMP_REGO_TYPE_X],
      x.[EXEC_EDUC],
      x.[EXEC_OFFIC_OTH],
      x.[EXEC_OFFIC_YN],
      x.[EXEC_FIRST_LIEN],
